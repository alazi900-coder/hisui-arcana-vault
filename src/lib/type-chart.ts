...
// Get weaknesses (types that deal 2x or 4x damage)
export function getWeaknesses(types: PokemonType[]): { type: PokemonType; multiplier: number }[] {
  const effectiveness = getDefensiveEffectiveness(types);
  return Object.entries(effectiveness)
    .filter(([_, mult]) => mult > 1)
    .map(([type, multiplier]) => ({ type: type as PokemonType, multiplier }))
    .sort((a, b) => b.multiplier - a.multiplier);
}

// Get resistances (types that deal 0.5x or 0.25x damage)
export function getResistances(types: PokemonType[]): { type: PokemonType; multiplier: number }[] {
  const effectiveness = getDefensiveEffectiveness(types);
  return Object.entries(effectiveness)
